$(function(){
        	...
        	var floors=$(".floor");
            var flo=$(".floors");
            // 用来获取每个楼层距离顶端的距离(按需加载)
        	var floorArr=[];
        	for(var i=0;i<floors.length;i++){
        		floorArr.push(floors[i].offsetTop);
        	}
            console.log(floorArr)
            // 用来获取每个楼层距离顶端的距离(楼层跳转)
            var floArr=[];
            for(var i=0;i<flo.length;i++){
                floArr.push(flo[i].offsetTop);
            }
            // 楼层跳转
            var jump=$(".sidebar")[0];
            var jumps=$("li",jump);
        	...
        	window.onscroll=function(){
                ...
                for(var i=0;i<jumps.length;i++){
                    if(scrolltop>floArr[i]-100){
                        for(var j=0;j<jumps.length;j++){
                            jumps[j].className="";
                        }
                        jumps[i].className="hot";
                    }
                }
            ...
            for(var i=0;i<jumps.length;i++){
                ...
                jumps[i].onclick=function(){
                    // 楼层跳转
                    var obj=document.body.scrollTop?document.body:document.documentElement ;
                    animate(obj,{scrollTop:floArr[this.index]-50}/*,function(){flag=true;}*/);
